...
/bin/bash -c "sed 's/_VERSION_/${VERSION}/' ${TARGET_DIR}/packaging/rpm/${NAME}.spec > /tmp/${NAME}.spec && \
              sed -i -e \"/^%changelog/a\* $(date '+%a %b %-d %Y') ${AUTHOR_EMAIL} - ${VERSION}-${RELEASE}\n- New vesion ${VERSION}\n\" /tmp/${NAME}.spec && \
              git archive --prefix=${NAME}-${VERSION}/ --format=tar main |gzip  >/tmp/${NAME}-${VERSION}.tar.gz
