...
def realhumanize(now: datetime.datetime, end_time: datetime.datetime) -> str:
    ...
    make_human_readable = lambda delta: [
        '%d %s' % (getattr(delta, attr), attr
                   if getattr(delta, attr) > 1 else attr[:-1])
...
def humanize_time(start_time: datetime.datetime,
    ...
    if start_time <= now <= end_time:
        return realhumanize(now - datetime.timedelta(minutes=1),
                            end_time) + " left"
    try:
    ...
    # remove plural
    if len(natural.split(
            " ")) == 2 and natural[0] == "1" and natural[-1] == "s":
        natural = natural[0:-1]
